...
namespace LolCode.Ast
{
    ...
    call i32 (i8 *, ...) @printf(i8* getelementptr inbounds ([{strNode.Value.Length + 1} x i8], [{strNode.Value.Length + 1} x i8]* @.str{strNode.StringTableIndex}, i64 0, i64 0))");
            ...
            else if (Children[0] is IdentifierNode identNode)
            {
                ...
                Console.WriteLine($"    %{r} = load {llvmType}, {llvmType}* %{identNode.Identifier}, align {alignment}");
                Console.WriteLine($@"
    call i32 (i8 *, ...) @printf(i8* %{r})");
        ...
        private void EmitInt()
        {
            ...
            {
                var r = Lolc.NextReg;
                Console.WriteLine($"    %{r} = add i32, 0, {intNode.Value}");
                Console.WriteLine($@"
    call i32 (i8 *, ...) @printf(i8* getelementptr inbounds ([3 x i8], [3 x i8]* @.percent_d, i64 0, i64 0), i32 %{r})");
            ...
            {
                var r = Lolc.NextReg;
                Console.WriteLine($"    %{r} = load i32, i32* %{identNode.Identifier}, align 4");
                Console.WriteLine($@"
    call i32 (i8 *, ...) @printf(i8* getelementptr inbounds ([3 x i8], [3 x i8]* @.percent_d, i64 0, i64 0), i32 %{r})");
            }
            else
        ...
        private void EmitFloat()
        {
            ...
            {
                var r = Lolc.NextReg;
                Console.WriteLine($"    %{r} = add double, 0, {floatNode.Value}");
                Console.WriteLine($@"
    call i32 (i8 *, ...) @printf(i8* getelementptr inbounds ([3 x i8], [3 x i8]* @.percent_f, i64 0, i64 0), double %{r})");
            ...
            {
                var r = Lolc.NextReg;
                Console.WriteLine($"    %{r} = load double, double* %{identNode.Identifier}, align 8");
                Console.WriteLine($@"
